...
for (const dir of [uploadsDir, rendersDir, jobsDir]) fs.mkdirSync(dir, { recursive: true });
...
function splitScenes(script) {
  ...
  for (const piece of pieces) {
    if (piece.length <= 430) scenes.push(piece);
    else {
...
async function runwayCreate(prompt, ratio) {
  ...
  catch { throw new Error(`Runway ने JSON response नहीं दिया (HTTP ${response.status}).`); }
  if (!response.ok) {
    throw new Error(`Runway API ${response.status}: ${data?.error || data?.message || text.slice(0, 500)}`);
  }
  if (!data?.id) throw new Error("Runway ने task ID नहीं दिया।");
  return data.id;
}
...
async function runwayStatus(taskId) {
  ...
  let data;
  try { data = JSON.parse(text); }
  catch { throw new Error(`Runway status ने JSON response नहीं दिया (HTTP ${response.status}).`); }
  if (!response.ok) {
    throw new Error(`Runway status ${response.status}: ${data?.error || data?.message || text.slice(0, 500)}`);
  }
  return data;
}

async function waitForTask(id, taskId, sceneIndex, sceneCount) {
  for (let attempt = 0; attempt < 120; attempt++) {
    ...
    setJob(id, {
      status: "generating",
      progress: Math.min(74, 5 + Math.round(((sceneIndex + attempt / 24) / sceneCount) * 70)),
      message: `AI scene ${sceneIndex} तैयार हो रहा है…`,
      currentTaskId: taskId
    });
    await new Promise(r => setTimeout(r, 5000));
  }
  throw new Error(`Runway scene ${sceneIndex} timeout हो गया।`);
}
...
async function generateScenes(id) {
  ...
  if (!job) return;
  try {
    const { scenes, format, style } = job;
    const ratio = format === "16:9" ? "1280:720" : "720:1280";
    const urls = Array.isArray(job.scenes) ? [...job.scenes] : [];
    let start = urls.length;
    ...
    for (let i = start; i < scenes.length; i++) {
      ...
      setJob(id, {
        ...
        message: `AI scene ${i + 1} of ${scenes.length} शुरू हो रहा है…`,
        error: null
      });

      let taskId = job.currentTaskId && i === Number(job.currentSceneIndex) ? job.currentTaskId : null;
      if (!taskId) {
        taskId = await runwayCreate(prompt, ratio);
        setJob(id, { currentTaskId: taskId, currentSceneIndex: i });
      }

      const task = await waitForTask(id, taskId, i + 1, scenes.length);
    ...
    setJob(id, {
      ...
      currentTaskId: null,
      currentSceneIndex: null,
      message: "AI scenes तैयार हैं। Final MP4 बनाया जा सकता है।"
    });
  } catch (error) {
    console.error("GENERATION ERROR:", error);
    setJob(id, { status: "error", progress: 0, error: errorText(error), currentTaskId: null });
  }
}
...
app.post(
  ...
  async (req, res) => {
    try {
      if (!runwayKey) return publicError(res, 500, "RUNWAYML_API_SECRET is not configured on the server.");
      const script = String(req.body.script || "").trim();
      const format = String(req.body.format || "9:16");
      const style = String(req.body.style || "Mystery");
      if (!script) return publicError(res, 400, "Script is required.");

      const scenes = splitScenes(script);
      if (!scenes.length) return publicError(res, 400, "Script could not be split into scenes.");

      const id = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
      const voice = req.files?.voice?.[0]?.path || null;
      const music = req.files?.music?.[0]?.path || null;

      setJob(id, {
        id, status: "generating", progress: 1, sceneIndex: 0,
        sceneCount: scenes.length, scenes: [], error: null,
        script, format, style, voice, music,
        currentTaskId: null, currentSceneIndex: null
      });
      ...
      publicError(res, 500, errorText(error));
    }
  }
);
...
app.get("/api/job/:id", (req, res) => {
  ...
  if (!job) return res.status(404).json({
    status: "not_found",
    error: "Job नहीं मिला। यह पुराना job हो सकता है। नया Generate करें।"
  });
  res.json(job);
});
...
async function renderJob(jobId) {
  ...
  if (!job || job.status !== "ready") throw new Error("AI scenes are not ready yet.");
  ...
  for (let i = 0; i < job.scenes.length; i++) {
    ...
    if (!fs.existsSync(p)) await downloadFile(job.scenes[i], p);
    sceneFiles.push(p);
    setJob(jobId, {
  ...
  const voice = job.voice && fs.existsSync(job.voice) ? job.voice : null;
  const music = job.music && fs.existsSync(job.music) ? job.music : null;
  ...
  if (voice || music) {
    ...
    const outputOptions = [
      ...maps, "-c:v", "libx264", "-preset", "veryfast",
      "-c:a", "aac", "-shortest"
    ];
    if (filters.length) outputOptions.push("-filter_complex", filters.join(";"));

    await ffmpegRun(cmd.outputOptions(outputOptions).output(final));
  ...
  setJob(jobId, {
    status: "done", progress: 100, video: videoUrl,
...
app.post(
  ...
  const job = jobs.get(jobId);
  if (!job) return publicError(res, 404, "Job नहीं मिला। नया Generate करें।");

  try {
    if (req.files?.voice?.[0]?.path || req.files?.music?.[0]?.path) {
      setJob(jobId, {
        voice: req.files?.voice?.[0]?.path || job.voice || null,
        music: req.files?.music?.[0]?.path || job.music || null
      });
    }
    const fresh = jobs.get(jobId);
    const video = fresh.status === "done" && fresh.video
      ? fresh.video
...
app.get("/{*splat}", (_req, res) => {
  ...
  if (!fs.existsSync(index)) return res.status(500).send("public/index.html not found on server");
  res.sendFile(index);
});
...
for (const [id, job] of jobs) {
  if (job.status === "generating" && job.currentTaskId) {
    console.log(`Resuming saved Runway job ${id}`);
    void generateScenes(id);
  }
}

app.listen(PORT, "0.0.0.0", () => console.log(`Raz Ki Duniya running on port ${PORT}`));
